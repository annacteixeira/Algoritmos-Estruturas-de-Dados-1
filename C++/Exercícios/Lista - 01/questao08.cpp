...
class Animal {
  ...
  public:
    ...
    void setNome(string nome) {
      if(nome.empty()) throw EntradaInvalidaException("\nO nome nao pode estar vazio");
      nome = nome;
    }

    void setRaca(string raca) {
      if(raca.empty()) throw EntradaInvalidaException("\nA raca nao pode estar vazia\n");
      raca = raca;
    }

    void setIdade(int idade) {
      if(idade < 0) throw EntradaInvalidaException("\nA idade nao pode ser negativa\n");
      idade = idade;
    }

    void setPreco(float preco) {
      if(preco < 0) throw EntradaInvalidaException("\nO preco nao pode ser negativo");
      preco = preco;
    }
